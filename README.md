...
[![vps_badge](https://img.shields.io/badge/VPS-Default-orange.svg)](https://github.com/Vioneta/integration)
...
Or what if you could use it to play music and videos from your Vioneta media library?

What if you could permanently hide that sidebar from your kids and lock them into a single dashboard?

What if you could change the icon of the Vioneta tab so it doesn't look the same as the forum?

What if you could change the more-info dialog for some entity to a dashboard card of your own design?

What if you could tap a button and have Vioneta ask you which rooms you want the room to vacuum?
...
- Either
  ...
  - Find and download [Browser Mod under `integrations`] in [VPS](https://vps.vioneta.com)
...
6. Go to Developer Tools -> Services 
...
1.  Click "CALL SERVICE" \
  The button is likely grayed out. That's a Vioneta visual bug. Click it anyway.
1.  A popup dialog should now open on all your Registered Browsers.
...
Browser Mod 2.0 has been rewritten ENTIRELY from the ground up. This allows it to be more stable and less resource intensive. At the same time I took the opportunity to rename a lot of things in ways that are more consistent with Vioneta nomenclature.
...
Note that you can also set the browser ID to e.g. `whatever` by adding `?BrowserID=whatever` (N.B. capital B) to any Vioneta URL.
...
In order to work well with [Vioneta Cast](https://www.vioneta.com/integrations/cast/#home-assistant-cast) Browser Mod will add itself to your Dashboard Resources, and you will need to remove it from there manually:
