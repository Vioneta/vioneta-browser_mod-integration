...
Will print `"Button was clicked in 79be65e8-f06c78f" to the Vioneta log.
...
To call a service from a dashboard use the call-service [action](https://www.vioneta.com/dashboards/actions/) or the special action `fire-dom-event`:
...
Point the browser to the given Vioneta path.
...
| | |
|---|---|
|`path` | A Vioneta path. <br/>E.x. `/lovelace/`, `/my-dashboard/bedroom`, `/browser_mod/`, `/config/devices/device/20911cc5a63b1caafa2089618545eb8a`...|
...
- `log(message)` - Print `message` to the Vioneta log
